 ...
 */
  public String toString(){
    ...
    for (int i=0; i<numInputAttributes; i++){
        ...
        switch(Attributes.getInputAttribute(i).getType()){
            case Attribute.NOMINAL:  
                val=nominalValues[0][i];
            	if (val==null) val = "?"; 
                aux += val;      
                break;

            case Attribute.INTEGER:
                val=(new Integer((int)realValues[0][i])).toString();
            	if (val==null) val = "?"; 
                aux += val;      
                break;
            case Attribute.REAL:               
                val=(new Double (realValues[0][i])).toString();
            	if (val==null) val = "?"; 
                aux += val;  
                break;
        }
    ...
    for (int i=0; i<numOutputAttributes; i++){
        ...
        switch(Attributes.getOutputAttribute(i).getType()){
            case Attribute.NOMINAL:
            	val=nominalValues[1][i]; 
            	if (val==null) val = "?";
                aux += val;      
                break;
            case Attribute.INTEGER: 
            	val=(new Integer((int)realValues[1][i])).toString();
            	if (val==null) val = "?"; 
                aux += val; 
                
                break;
            case Attribute.REAL:
            	val=(new Double(realValues[1][i])).toString();
            	if (val==null) val = "?"; 
                aux += val; 
                break;
        }
