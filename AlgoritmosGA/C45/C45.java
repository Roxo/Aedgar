...
public class C45 extends Algorithm {
    ...
    /** Confidence level. */
    private static float confidence = 0.25f;

    /** Minimum number of itemsets per leaf. */
    private static int minItemsets = 2;
    ...
    public static void main(String[] args) {
        try {
            if (args.length == 9) {
            	...
            	testFileName=args[2];	
            	trainOutputFileName=args[3];
            	testOutputFileName=args[4];
            	resultFileName=args[5];
            	
            	if(args[6].toLowerCase().equals("true")){
            		prune=true;
            	}else{
            		prune=false;
            	}
            	...
            	try{
            	 	confidence=Float.parseFloat(args[7]);
                 }catch (Exception e) {
                	 confidence=Float.parseFloat("0.1");
            	}
            	
             	try{
             		minItemsets=Integer.parseInt(args[8]);
