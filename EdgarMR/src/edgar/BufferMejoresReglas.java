 ...
 * Este por lo tanto esta accesible  travns del nombre de la clase con un metodo estntico 
 */
public class BufferMejoresReglas {
	 ...
	 */
	public synchronized int get_tamano_bufferReglas(){
		return buffer_comunicacion_mejores_reglas.getTamano_solucion();
	}
	/** 
	 * Anade una regla al buffer de comunicacinn con el supervisor
	 * comprueba que no exista ya antes de anadirla
	 */
	...
	public  Solucion get_mejores_reglas(){
		...
		for(int i=0;i<buffer_comunicacion_mejores_reglas.getTamano_solucion();i++){
