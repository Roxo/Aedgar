...
/**
 * Este mntodo lee de un fichero el conjunto de ejemplos y lo introduze en una estructura poblacion
 ...
 */
	public Dataset Cargar_conjuntoEntrenamiento(String NombreFichero, boolean esTrain){	
		...
		try {
			...
			if(esTrain)
			{
				// Datos lendos, nueva discretizacinn
				...
				for (int i = 0; i < numAtributos; i++) 
				{
					...
					else
					{
						...
						num--; // Hay un punto de corte menos que el nnmero de intervalos
	...
	/**
	 * El siguiente mntodo genera un fichero de datos seleccionando aleatoriamente
	 * un conjunto de ejemplos del fichero de entrada.
	 * @param NombreFichero_In Nombre del fichero con el conjunto de ejemplos.
	 * @param NombreFichero_Out Nombre del fichero que se va ha crear con la seleccinn de un subconjunto de ejemplos
	 * @param Numero_Ejemplos_Selecionar Nnmero de ejemplos a seleccionar.
	 */
	...
	/**
	 * Este mntodo genera un fichero de salida, con dos columnas, una que indica la clase de salida y la otra la prediccinn realizada por el clasificador.
	...
	public void generar_Fichero_Reglas(String NombreFichero_Reglas, Solucion conjunto_reglas){	
		try {
		     ...
		     cabecera+= "Nnmero de Reglas: "+ conjunto_reglas.getTamano_solucion()+"\n";
		     f.write(cabecera.getBytes());
		     for(int i=0;i<conjunto_reglas.getTamano_solucion();i++){
