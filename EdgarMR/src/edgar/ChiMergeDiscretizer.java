...
public class ChiMergeDiscretizer extends Discretizer {
	...
	protected Vector discretizeAttribute(int attribute,int []values,int begin,int end) {
		...
		double threshold=critchi(confidenceThreshold
			,Parametros.getInstancia_Parametros().getNumeroDeClases()-1);
		...
		while(intervals.size()>1 && !exit) {
			...
			for(int i=0;i<intervals.size()-1;i++) {
				...
				for(int j=0;j<int1.cd.length;j++) {
					if(j == getClaseMinoritaria())
					{
						int2.cd[j] *= ponderacion;
						int1.cd[j] *= ponderacion;
	...
	int []classDistribution(int attribute,int []values,int begin,int end) {
		...
		int numClases = Parametros.getInstancia_Parametros().getNumeroDeClases();
		int []classCount = new int[numClases];
		for(int i=0;i<numClases;i++) classCount[i]=0;
	...
	/*
	// No se utiliza, se utiliza el "mejorado" que se llama igual pero tiene más parámetros
	@Override
