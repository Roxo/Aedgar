...
public class Net_Estrella implements NET{
	...
	// Constructor privado, para asegurarnos que solo existe un buffer en toda la aplicacinn.
	...
	public void EnviarDatosEntrenamiento(int ident_Nodo, Dataset nuevosDatosEntrenamientos){
		synchronized(buffer_datos_entrenamientos){
			param.depura("Nodo("+ident_Nodo+") ENVIA DATOS "+nuevosDatosEntrenamientos.getTamano_conjunto_entrenamiento() +" NET ANILLO ..." ,1);	
			
			for (int i=0;i<nuevosDatosEntrenamientos.getTamano_conjunto_entrenamiento();i++)
	...
	public  Dataset Get_Datos_Entrenamiento (int Nodo){
		...
		synchronized(buffer_datos_entrenamientos){
			if(buffer_datos_entrenamientos.getTamano_conjunto_entrenamiento() >0){
				// Primero asigno los datos de entrenamiento
				int particion = buffer_datos_entrenamientos.getTamano_conjunto_entrenamiento()/param.get_Numero_Nodos();
				...
				param.depura("Nodo(" +Nodo + "): Ha obtenido "+ datosEntrenamiento.getTamano_conjunto_entrenamiento()+" Datos de entrenamiento ...",2);
			}
		}
