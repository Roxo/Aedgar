...
public class Pool extends Thread implements i_Supervisor{
	...
	protected Solucion recibeReglas(BufferMejoresReglas reglasEnviadas,Dataset ejemplos_aux){
		...
		param_globales.depura(" Pool : Ha recibido de los nodos --> " + Anet.getTamano_solucion() + " reglas.",0);	
...
// 			devuelve  el concepto ampliado
	...
	public void run(){
		...
		while(buscando){
			...
			concepto = recalculaConcepto(concepto,reglas_Recibidas,ejemplos_aux);
				...
				param_globales.depura(" Supervisor nnmero Reglas del concepto actual: " +concepto.getTamano_solucion(),-1);
				param_globales.depura("Casos no cubiertos: "+ concepto.get_Ejemplos_No_Cubiertos(ejemplos).getTamano_conjunto_entrenamiento(),-1);
		...
		if (Parametros.getInstancia_Parametros().getNodosActivos()== 0) {
			...
			//Snlo optimiza si hay algnn atributo numnrico
			if (param_globales.getOptimizaParticiones() && ((Regla)(concepto.getReglas().get(0))).plantilla.numeroAtributosNumericos() > 0)
			{
			  ...
			  // chc --n modificando  los puntos de corte  y el fitness : calcular la clasificacinn sobre training. 
			// cromosoma : plantilla
	/*
	 * poblacion 30 plantillas creadas por mutacinn de la primera
	 *   Seleccinn por torneo-- 5--: coge 5 aleatorias y se queda con la mejor y tiene un padre.
	 *   genera 30 hijos que son la nueva generacion (generacional)
	 *   reinicializacnn: sin son muy parecidos (distancia hamming) -n mo se pueden cruzar --n si no se puede cruzar ninguno, se regenera la poblacinn ( con el mejor) 
	 *   Operador de cruce:  blx-Alpha , 0,5 gebera un nuevo numero que estarn entreun minimo-alpha y maximo+alpha,
	 */
			Solucion reglas_Recibidas= recibeReglas(BufferMejoresReglas.getInstancia_buffer(),ejemplos_aux);
			...
			for(int i=0;i<concepto.getTamano_solucion();i++){
	...
	///////////////////////////////////////////
	// CnDIGO DE JOSn MANUEL GARRIDO MORGADO //
	///////////////////////////////////////////
