 ...
 * 
 * 
 *	Esta clase implementa los operadores gennticos que utilizan los nodos.
 */
/**
 * @author Miguel Angel
 *
...
public class Operador {
	 ...
	 * pero snlo de parte de la regla , para hacer exploracinn de la parte de atributos que le corresponda
	...
	public static Regla Sembrado(EjemploFuzzy votante,int inicio,int numAtributos){
		...
		for(int i_atr=inicio;i_atr<numAtributos;i_atr++){
				//Atributo de entrada
				// Si el atributo estn evaluado, hacemos que cumpla el ejemplo
				if (NuevaRegla.atributo_evaluado(i_atr)){
					int ind_aux=votante.getPlantilla().posicionAtributo(i_atr);
					// Cambio a 1 el mnnimo nnmero de valores
	...
	/**
	 * Operador de sufragio universal, Operador de seleccinn. 
	 * Selecciona el conjunto de reglas que se van a  participar en la reproduccinn.
	 ...
	 * @param m tamano de la poblacinn.
	 * @return Conjuntos de individuos seleccionados para la reproduccinn.
	 */
	...
	public static Solucion US(Dataset datosEntrenamiento,Solucion Reglas,int m){
		...
		for(int i=0;i<datosEntrenamiento.getTamano_conjunto_entrenamiento();i++) Indices_Poblacion_Total.add(new Integer(i));
		
		if(num_Individuos_Votan<datosEntrenamiento.getTamano_conjunto_entrenamiento()){
			...
			for(int i=0;i<num_Individuos_Votan;i++){
				...
				Indices_Poblacion_Total.remove(indice_aleatorio); // Versinn v1.2 Sino el mismo ejemplo puede votar varias veces
				
			}
		}else{  // En el caso de que queden menos o igual nnmero de ejemplos que ejemplos hay que selecionas
			//num_Individuos_Votan=ejemplos.getTamano_conjunto_entrenamiento();
			
			num_Individuos_Votan=datosEntrenamiento.getTamano_conjunto_entrenamiento();
			Individuos_Votan=new int[num_Individuos_Votan];
		...
		for (int i=0;i<num_Individuos_Votan;i++){
			...
			int indice_votante=Individuos_Votan[i];
	         ...
	         if (indice_votante > datosEntrenamiento.getTamano_conjunto_entrenamiento())
			...
			for (int j=0;j<Reglas.getTamano_solucion();j++){
			...
			if (reglas_cubren_Ejemplo.getTamano_solucion()>0){
	...
	public static Solucion US_sinSembrado(Dataset ejemplos,Solucion Reglas,int m){
		...
		for(int i=0;i<ejemplos.getTamano_conjunto_entrenamiento();i++) Indices_Poblacion_Total.add(new Integer(i));
		
		if(num_Individuos_Votan<ejemplos.getTamano_conjunto_entrenamiento()){
			Individuos_Votan=new int[num_Individuos_Votan];
			for(int i=0;i<num_Individuos_Votan;i++){
				int indice_aleatorio=param_globales.get_GeneradorAleatorio().Randint(0,Indices_Poblacion_Total.size()-1);
				Individuos_Votan[i]=Integer.parseInt(Indices_Poblacion_Total.get(indice_aleatorio)+"");
		...
		for (int i=0;i<num_Individuos_Votan;i++){
			// Obtengo el votante
			int indice_votante=Individuos_Votan[i];
	         if (indice_votante > ejemplos.getTamano_conjunto_entrenamiento())
...
/**
 * Este mntodo se utiliza para decidir qun tipo de cruce se va a utilizar.
 ...
 */
	...
	public static int Selector_Tipo_Cruce(Regla parent1, Regla parent2, Dataset ejemplos){
			...
			r=((double)(n_positivo_parent1+n_negativo_parent1+n_positivo_parent2+n_negativo_parent2)/(double)(ejemplos.getTamano_conjunto_entrenamiento()^2));
			Parametros param_globales=Parametros.getInstancia_Parametros();
	...
	/**
	 * Este mntodo es el encargado de aplicar el cruce entre dos individuos seleccionados.
	 * @param padre1 Individuo a cruzar.
	 * @param padre2 Individuo a cruzar.
	 * @param hijo1 Este es un parnmetro que se pasa por referencia, y el se devuelve un hijo generado por el cruce.
	 * @param hijo2 Este es un parnmetro que se pasa por referencia, y el se devuelve un hijo generado por el cruce.
	 * @param ejemplos
	 ...
	 * en los parnmetros hijo1 y hijo2, se devuelven las dos nuevas reglas generadas.
	 * @param Padre1 
	 * @param Padre2
	 * @param Hijo1
	 * @param Hijo2
...
/**
 * Cruce Uniforme, A este mntodo se le pasa dos reglas padre1 y padre2, y genera dos reglas hijas, donde cada bit tiene
 ...
 */
	...
	/**
	 * Este mntodo aplica la mutacinn al individuo que se le pasa por referencia, 
...
/*
 * 
 * CnDIGO CREADO POR JOSn MANUEL GARRIDO MORGADO
 * 
 */

	public static Solucion TokenCompetition(Dataset ejemplos,Solucion Reglas,int m)
	{
		int num_reglas = Reglas.getTamano_solucion();
		int num_ejemplos = ejemplos.getTamano_conjunto_entrenamiento();
				...
				for(int z=0;z<i;z++)
				{
					...
					if(r_contrincante.Cubre_Ejemplo(e_actual)>Parametros.getInstancia_Parametros().getCoberturaFuzzy())
					{
						if(fitness_r_actual < r_contrincante.evaluar_Fitness())
						{
							...
							// Si no posena este ejemplo, se lo asignamos como suyo
		...
		for(int i=0;i<num_reglas;i++)
		{
			// Metemos en el HashMap el nndice de la regla (la i) y su fitness, para luego ordenarlas mns fncilmente
