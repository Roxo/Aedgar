...
import edgar.*;
 ...
 /**
* Esta clase realiza una ejecucinn con los parametros recibidos por linea de comando.
* usage: 
* Almacena la salida en la carpeta de resultados con el nombre del fichero de datos mns la fecha de ejecucinn.
...
* La salida estn compuesta por los ficheros de entrenamiento y test utilizados, reglas generadas, resultado (copia de salida por pantalla) 
*  y resumen de resultado separado por comas para su importacinn a fichero csv en excel . 
...
public class Principal   {
	...
	public static void ejecutar(String nombre_fichero_Entrenamiento,
		...
		.Cargar_conjuntoEntrenamiento(Nombre_fichero_test, false);
			...
			/*v2
	    	 *  Modificar dinamicamente el nnmero de nodos y la poblacinn por balanceo de clases. 
	    	 *  se copia a cada nodo la clase minoritaria y se calcula el nnmero de nodos por el ratio entre la mayoritaria y la minoritaria.
	    	 *  Minimo de nodos = 2; Mnximo de nodos 16.
	    	 *  La poblacinn se calcula con un ratio de 1 regla por ejemplo mnximo (para datasets muy pequenos), 
	    	 *  y ratio ideal de 10 ejemplos por regla con un mnximo de 200.
	    	 */
	    	if (Parametros.getInstancia_Parametros().isBalanceoClases()){
	    		//Ratio de balanceo es la mayoritaria entre minoritaria. El nnmero de nodos es ese nnmro.
	    		int tempNodos= 0;
		...
		//Se queda esperando aqun hasta que la solucinn final estn disponible con un wait
		...
		Texto_Resultado += "   --- RESULTADOS  dna: " + FechaHora
				+ " ---\n\n";

		Texto_Resultado += "Nnmero de hilos: " + param.get_Numero_Nodos()
				+ "\n";
		double ejemplosNodo = (param.get_repartir_entrenamiento_nodos()==true ? (ejemplos_train.getTamano_conjunto_entrenamiento() / param
				.get_Numero_Nodos()) : ejemplos_train.getTamano_conjunto_entrenamiento());
		...
		Texto_Resultado += "\nTamano Entrenamiento:      "+ejemplos_train.getTamano_conjunto_entrenamiento()
		...
		Texto_Resultado += "\n";
		Texto_Resultado += "Nnmero de Reglas: " + conjunto_reglas.getTamano_solucion();
		Texto_Resultado += "\n\n\n";
		...
		Texto_Resultado += "***  Matriz de confusinn Test  ***\n";
		...
		Texto_Resultado += "***  Matriz de confusinn Entrenamiento  ***\n";
		for (int aux = 0; aux < param.getPlantilla().get_numero_Clases(); aux++)
			Texto_Resultado += "      " + param.getPlantilla().get_Valores_Clase().get(aux);
		Texto_Resultado += "\n";
		for (int aux = 0; aux < param.getPlantilla().get_numero_Clases(); aux++) {
		...
		minutos_empleados + "," + ejemplos_train.getTamano_conjunto_entrenamiento() + "," + _Num_Evaluaciones +"," +  _Num_EvaluacionesDesglosado +
		","+ _Num_Comunicaciones +  "," + conjunto_reglas.getTamano_solucion() + ","+ (numAciertosTst / (double) ResultadoClasificaciontst.length)+ 
	...
	public static void main(String[] args) {
		...
		if (args.length>0){
	    	...
	    	System.out.println(" Topologna: " + args[5]);
	        }
