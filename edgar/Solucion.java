...
public class Solucion {
	...
	/**
	 * El siguiente método introduce una regla en el conjunto de reglas de forma ordenada
	...
	public Solucion copia(){
		...
		int num_reglas = this.getTamaño_solucion();
	...
	/**
	 * El siguiente método calcula la regla de mejor PI
	...
	public Dataset get_Ejemplos_No_Cubiertos(
		...
		while ((indice_ejemplos_No_Cubiertos.size() > 0)
				&& (cont_reglas < getTamaño_solucion())) {
 ...
 */
	public Dataset get_Ejemplos_Poco_Cubiertos(
		...
		int num_reglas = getTamaño_solucion();
		...
		{		
			...
			while(cont_reglas<num_reglas && (!encontrado))
			{
				...
				if (Parametros.getInstancia_Parametros().isFuzzy())
				{
					...
					if(tabla_h0[cont_reglas] > mayor_h0)// método gana la mejor.
					{
			...
			//ojo varios metodos Fuzzy:
				// 1º   evaluar todos los h0 y coger la regla con el mayor h0
				// 2º  SUmar agregando igual que en torcs y se desfuzzyfica al final y la etiqueta que de es la clase
				// 3º cn2--> definir un h0 min. h0 < 0.2 , es como 2 , por ejemplo si aplican tres reglas: h01 .7, h02 .3 , h03 .2 -->
				// clase final --> 0,7 * clase1ª + 0.3*(0.2*clase 2ª + ... , pdte enviar formula.
			...
			{
			////dos metodos--> 
			//a)quedarnos con el mayor valor relativo (ya implementado en el while anterior) y esta siguiente línea
			    clase=regla_elegida.getClase();
			}
			else//b) sumar en un array las n clases los valores acumulados -> Es el siguiente bloque de código entre corchetes
			{
...
// TODO Plantilla no se está usando. Corregir.	
	...
	public void quitaAtributosNoSelectivos(){
		for (int i=0;i < this.getTamaño_solucion();i++)
	...
	public boolean Poblaciones_Iguales(Solucion sol1, Solucion sol2){	
		...
		if(sol1.getTamaño_solucion()!=sol2.getTamaño_solucion()) iguales=false;		
		if(iguales){
			int i=0;
			while((iguales)&&(i<sol1.getTamaño_solucion())){
	...
	public boolean Es_Igual(Solucion sol){	
		boolean iguales=true;		
		if(getTamaño_solucion()!=sol.getTamaño_solucion()) iguales=false;		
		if(iguales){
			int i=0;
			while((iguales)&&(i<getTamaño_solucion())){
	...
	public String get_texto_solucion_Completa(){
		...
		for(int i=0;i<getTamaño_solucion();i++){				
			text_solucion+=(i+1)+".- ";
	...
	/**
	 * Esta función devuelve el concepto en el conjunto de ejemplos enviados como parametro. 
	...
	private Solucion calculaConceptoGreedy(Dataset datosEntrenamiento){
		...
		while((indice_ejemplos_No_Cubiertos.size()>0)&&(cont_reglas<getTamaño_solucion() ) ){				
			Regla regla=get_regla(cont_reglas);
			...
			if(!Parametros.getInstancia_Parametros().isFuzzy())
			{
				while(i<indice_ejemplos_No_Cubiertos.size()){
					...
					// ojo la que está escrita es la versión discreta , los cambios hechos son para la versión fuzzy. hacer un if es fuzzy else
					if(regla.Cubre_Ejemplo_Pos(ej)) {
	...
	////////////////////////////////////////////////////////////
	//                                                        //
	//    A PARTIR DE AQUÍ SON FUNCIONES PROGRAMADAS POR MI   //
	//                                                        //
	////////////////////////////////////////////////////////////
	...
	private Solucion calculaConceptoGeneticoGeneracional(Dataset conjunto_ejemplos)
	{
		...
		// Van a ser un número aleatorio entre 1 y 100
		...
		{
			// Generamos el número de reglas que va a tener el individuo que vamos a generar
			int num_reglas = al.nextInt(num_reglas_conseguidas)+1; // Van a ser entre 1 y el número total de reglas que tengamos
			...
			// Seleccionamos el mejor y lo guardamos para no perder el óptimo
		...
		{				
			...
			// Generar la nueva población mediante selección aleatoria de padres, cruce y mutación
			...
			{
				ArrayList padre1 = torneo(5, poblacion_antigua, conjunto_ejemplos, num_reglas_conseguidas, al);//(ArrayList) poblacion_antigua.get(al.nextInt(num_individuos_actuales));//MARR 2011/01/14 Metodo selección --> Torneo 5
				...
				// En la función cruce ya está considerado que devuelva un individuo válido
	...
	private Solucion calculaConceptoGeneticoNoGeneracional(Dataset conjunto_ejemplos, Solucion solGreedy)
	{
		...
		for(int i=1;i<num_individuos;)
		{
			...
			if (num_reglas_conseguidas < 4)
				...
				num_reglas = al.nextInt(num_reglas_conseguidas-3)+3; // Van a ser entre 3 y el número total de reglas que tengamos
			ArrayList individuo = new ArrayList();
			for (int j=0;j<num_reglas;)
			{
				// Seleccionamos de manera aleatoria la regla a insertar
		...
		{				
			ArrayList padre1 = torneo(10, poblacion, conjunto_ejemplos, num_reglas_conseguidas, al);//(ArrayList) poblacion_antigua.get(al.nextInt(num_individuos_actuales));//MARR 2011/01/14 Metodo selección --> Torneo 5
			...
			// Vemos si es peor que el recién creado, 
			// y si el recién creado es mejor que el peor de estos 10 aleatorios,
	...
	public ArrayList mutacion_antigua(ArrayList individuo, Random al)
	{
		...
		if(aleatorio < 0.05  && longitud > 2)//0.05 es el porcentaje de mutación. 
		{
			// Vamos a hacer entre 1 y longitud/2 cambios
			int num_cambios = al.nextInt(longitud/2) + 1;
			
			// Los índices al principio harán referencia a sus mismas posiciones
			...
			// Hacemos los cambios en los índices
	...
	*/
	...
	public ArrayList cruce_en_dos_puntos(ArrayList lista1, ArrayList lista2, Random al, double porcentaje_cruce, Dataset conjunto_ejemplos)
	{
		...
		// maximo es el punto máximo por donde podemos hacer el segundo corte
		// mitad_maximo es el punto máximo por donde haremos el primer corte
		int mitad_maximo = maximo/2;
		
		// Si son dos listas muy pequeñas, devolvemos la primera (no hay cruce)
		if(mitad_maximo < 2)
			return lista1;
		...
		for(int i=0;i<primer_corte;i++)
		{
			...
			else
			{
				...
				// está en el trozo intermedio que vamos a meter del segundo padre
